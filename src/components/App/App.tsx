...
let chainId = await ethereum.request({ method: 'eth_chainId' });
  ...
  const setupEventListener = async () => {
    try {
      ...
      if(ethereum) {
        ...
        const connectedContract = new ethers.Contract(CONTRACT_ADDRESS, ssgNft.abi, signer);
  ...
  <div>   
    
    {
      ...
      : (
        <>
        <button onClick={askContractToMintNft} className="cta-button mint-button connect-wallet-button">
          Mint NFT
        </button>
        <p>Price: FREE ✨ (+ gas)</p>
        </>)
    ...
    <div className="App">
      <div className="container">
        <div className="header-container">
          <p className="header gradient-text">(S)mooth (S)quare (G)radient NFT Collection</p>
        ...
        <div>
        {nftLink ? 
          <a href={nftLink} target="_blank" rel="noreferrer">See your NFT 👀</a>
          : null
        }
        </div>
        <div className="">
          <p>Number of minted NFTs: {mintedAmount}/{TOTAL_MINT_LIMIT}</p>
          <a href={OPENSEA_LINK} target="_blank" rel="noreferrer" >🌊 View collection on OpeanSea </a>
        </div>
